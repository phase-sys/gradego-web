...
import { Home, Calendar, Users, BookOpen, Settings, Menu, Zap, ClipboardList, MessageSquare, Download, LayoutDashboard, GraduationCap } from 'lucide-react';
...
const TeacherNavItems = [
  { to: "/teacher/dashboard", label: "Dashboard", icon: <Home />, group: "Core Tools" },
  { to: "/teacher/planner", label: "Planner", icon: <Calendar />, group: "Core Tools" },
  { to: "/teacher/attendance", label: "Attendance", icon: <ClipboardList />, group: "Core Tools" },
  { to: "/teacher/randomizer", label: "Randomizer", icon: <Zap />, group: "Core Tools" },
  
  { to: "/teacher/classes", label: "Classes", icon: <Users />, group: "Management & Data" },
  { to: "/teacher/assessments", label: "Assessments", icon: <BookOpen />, group: "Management & Data" },
  { to: "/teacher/suggestions", label: "Suggestions", icon: <MessageSquare />, group: "Management & Data" },
  { to: "/teacher/export", label: "Export Data", icon: <Download />, group: "Management & Data" },
];
...
const MobileLayout: React.FC<MobileLayoutProps> = ({ children }) => {
  ...
  const bottomNavItems = TeacherNavItems.filter(item => 
    ["/teacher/dashboard", "/teacher/planner", "/teacher/classes", "/teacher/assessments", "/teacher/attendance"].includes(item.to)
  ).slice(0, 5);
        ...
        <Sheet>
          ...
          <SheetContent side="right" className="w-[250px] sm:w-[300px]">
            <nav className="flex flex-col space-y-4 pt-8">
              ...
              <p className="font-semibold px-2">Welcome, {user?.name || 'Teacher'}!</p>
              <Separator />
              
              <Accordion type="multiple" defaultValue={["Core Tools", "Management & Data"]} className="w-full">
              ...
              <Separator />
              <Link href="/teacher/settings" className="flex items-center space-x-3 p-2 rounded-md hover:bg-muted">
